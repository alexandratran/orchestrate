...
func (s *schedulesCtrlTestSuite) TestScheduleController_Create() {
	scheduleRequest := apitestdata.FakeCreateScheduleRequest()
	...
	s.T().Run("should execute request successfully", func(t *testing.T) {
		...
		scheduleEntityResp := testdata.FakeSchedule()
...
func (s *schedulesCtrlTestSuite) TestScheduleController_GetAll() {
	s.T().Run("should execute request successfully", func(t *testing.T) {
		...
		schedulesEntities := []*entities.Schedule{testdata.FakeSchedule()}

		s.searchSchedulesUC.EXPECT().Execute(gomock.Any(), s.userInfo).Return(schedulesEntities, nil)

		s.router.ServeHTTP(rw, httpRequest)

		response := []*apitypes.ScheduleResponse{formatters.FormatScheduleResponse(schedulesEntities[0])}
		expectedBody, _ := json.Marshal(response)
		assert.Equal(t, string(expectedBody)+"\n", rw.Body.String())
		assert.Equal(t, http.StatusOK, rw.Code)
	})
