...
func Test_AfterNewBlock(t *testing.T) {
	...
	var fakeJobs = []*entities.Job{testdata.FakeJob()}
	...
	contract := apitestdata.FakeContractResponse()
	...
	t.Run("should process after new block successfully", func(t *testing.T) {
		...
		client.EXPECT().SearchContract(gomock.Any(), gomock.Any()).Return(contract, nil)
		client.EXPECT().GetContractEvents(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(&apitypes.GetContractEventsBySignHashResponse{
		...
		client.EXPECT().UpdateJob(gomock.Any(), gomock.Any(), &apitypes.UpdateJobRequest{
